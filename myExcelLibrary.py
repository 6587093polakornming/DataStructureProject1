...
ID: 6587093         SEC: 1 
level of program = high (มีการใช้งานโครงสร้างข้อมูลที่ซับซ้อน)"""
#module
...
class myExcel:
    ...
    class Row:                              #Class Row โครงสร้าง Hash table (!สมมติว่าเป็น prefect hash table)
        def __init__(self,numCol):          
            ...
            #หมายเหตุ ord() เป็น functionที่เปลี่ยนค่าอักษรเป็นค่าตัวเลขจำนวนจริง ตามหลัก ascii-table เช่น ord(A)=65
    ...
    def display_formula(self):
        """method ที่ใช้แสดงผลรูปแบบ แสดงค่าสูตร(display formula) ของตาราง myExcel"""
...
class ExpTree: 
    """โครงสร้างข้อมูลต้นไม้อย่างหนึ่งใช้ในการคำนวณสูตรคณิตศาสตร์เบื่องต้น"""
...
def make_list_from_string(string):
    """function ที่มีหน้าที string format input: "=A1+1.2" output: ['=', 'A', '1', '+', '1.2']"""
    out = re.findall(r'[A-Za-z]+|\d*\.\d+|\d+|\W', string)
    return out

def matching_paren(string): 
    """function ตรวจสอบความถูกต้องของวงเล็บ เป็น text processing อย่างหนึ่ง"""
...
def is_outOfRange(Excel:myExcel,check):
    """ function ตรวจสอบเงื่อนไขการอ้างอิงเกินขนาดของ myExcel return boolean"""
...
class Graph: 
    """โครงสร้างข้อมูลอ้างอิงจากทฤษฎีกราฟ เพื่อใช้ตรวจสอบความถูกต้องของการอ้างอิงวงกลม (circular reference)"""
...
def fixNamecell(stringlist:list):
    """#function. แก้ไข list หลังจากใช้งาน function make_list_from_string input :['A', '1', '+', '1.2']
output:['(', 'A 1', '+', '1.2', ')']"""
    out=list()
